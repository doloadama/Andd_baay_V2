    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {harvests.map(harvest => (
          <div key={harvest.id} className="bg-white dark:bg-gray-900 rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
            <img src={`https://picsum.photos/400/200?random=${harvest.id}`} alt={harvest.cropType} className="w-full h-40 object-cover" />
            <div className="p-6">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{harvest.cropType}</h3>
              <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">
                Listed by <span className="font-semibold">{harvest.farmer.name}</span> from <span className="font-semibold">{harvest.farmer.location}</span>
              </p>
